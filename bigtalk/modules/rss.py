...
from bigtalk.defines import Base, Disk, Object, Broker, Locate
from bigtalk.defines import Main, Method, Repeater, Thread, Utils, e
...
class Fetcher:
    ...
    def start(self, repeat=True):
        ...
        State.seenfn = Locate.last(State.seen) or Method.ident(State.seen)
        State.modifiedfn = Locate.last(State.modified) or Method.ident(State.modified)
        if repeat:
    ...
    def fetch(self, fnm, feed, silent=False):
        with Run.fetchlock:
            ...
            if silent:
                return counter
            if not State.seenfn:
                State.seenfn = Method.ident(State.seen)
    ...
    with Run.importlock:
        ...
        for obj in prs.parse(txt, "outline", "name,xmlUrl"):
            ...
            if not url.startswith("http"):
                continue
            has = list(Locate.find(Method.fqn(Rss), {"rss": url}, matching=True))
            if has:
...
def nme(event):
    ...
    for fnm, fed in Locate.find(Method.fqn(Rss), selector):
        feed = Rss()
...
def res(event):
    if len(event.args) != 1:
        ...
        return
    nrs = 0
    for fnm, fed in Locate.find(Method.fqn(Rss), removed=True):
        feed = Rss()
        Object.update(feed, fed)
        if event.args[0] not in feed.rss:
            continue
...
def rss(event):
    ...
    if "http://" not in url and "https://" not in url:
        ...
        return
    for fnm, result in Locate.find(Method.fqn(Rss), {"rss": url}):
        if result:
